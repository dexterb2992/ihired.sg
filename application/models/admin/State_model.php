...
class State_model extends CI_Model{
	...
	public function check($name, $country_id){
		$this->db->select("*");
		$this->db->where('LOWER(state_name)', "'".strtolower($name)."'", FALSE);
	...
	public function getByCountry($page = 1, $term = '', $country_id){
		...
		$this->db->select('state_name as text, '.$this->table_id.' as id');
		$this->db->like('state_name', $term);
		$this->db->where('country_id', $country_id, false);
